...
func TestTimeoutIsUnknownAndNotifiesOnlyAfterPersistence(t *testing.T) {
	...
	if repository.items[0].State != StateUnknown || !notified {
		t.Fatalf("state=%s notified=%v", repository.items[0].State, notified)
	}
}
...
func TestQueueDrainsAndNotifiesAfterMixedTerminalAggregate(t *testing.T) {
	...
	if calls != 1 || repository.items[0].State != StateSent || repository.items[1].State != StateSent {
		t.Fatalf("calls=%d states=%s,%s", calls, repository.items[0].State, repository.items[1].State)
	}
}
...
func (f *fakeRepository) ClaimNext(context.Context, string) (*Item, error) {
...
func (f *fakeRepository) PersistOutcome(_ context.Context, outcome Outcome) error {
	for _, item := range f.items {
...
func (f *fakeRepository) HasPending(context.Context, string) (bool, error) {
	for _, item := range f.items {
		if item.State == StatePending {
			return true, nil
		}
...
func (f *fakeRepository) Aggregate(context.Context, string) (ParentState, error) {
	return ParentCompleted, nil
}

type fakeProvider struct{ err, errOnce error }

func (f fakeProvider) Deliver(context.Context, Item) error {
	if f.errOnce != nil {
		return f.errOnce
	}
	return f.err
