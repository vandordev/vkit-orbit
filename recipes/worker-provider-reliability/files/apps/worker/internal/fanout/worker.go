...
func (w Worker) Process(ctx context.Context, parentID string) error {
	...
	if err := w.Repository.PersistOutcome(ctx, outcome); err != nil {
		return err
	}
	...
	if pending && w.Enqueue != nil {
		if err := w.Enqueue(ctx, parentID); err != nil {
			return err
		}
	}
	...
	if !pending && (state == ParentCompleted || state == ParentFailed || state == ParentPartiallyFailed) && w.Notify != nil {
		if err := w.Notify(ctx, parentID); err != nil {
			return fmt.Errorf("notify completed fanout: %w", err)
		}
	}
