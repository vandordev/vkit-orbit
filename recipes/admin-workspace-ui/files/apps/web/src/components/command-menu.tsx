	...
	return (
		<>
			<button
				...
				className="rounded-full border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-600 hover:border-slate-900 hover:text-slate-950"
			>
			...
			{open && (
				...
				>
					<div
						className="mx-auto mt-[12vh] max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"
						...
						<div className="p-2">
							{items.map((item) => (
								<a
									key={item.to}
									href={item.to}
									className="flex items-center rounded-xl px-4 py-3 text-sm text-slate-700 hover:bg-amber-50 hover:text-slate-950"
									onClick={() => {
										onNavigate?.(item.to);
										setOpen(false);
									}}
