...
export function RealtimeExamplePage({ createSocket = createRealtimeSocket }: { createSocket?: typeof createRealtimeSocket }) {
	...
	return (
		<main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-8 px-6 py-16">
			<div>
				<p className="text-sm font-medium uppercase tracking-[0.2em] text-muted-foreground">Opt-in realtime example</p>
				<h1 className="mt-3 text-3xl font-semibold tracking-tight">Socket.IO signal invalidation</h1>
				<p className="mt-3 text-muted-foreground">
					This walkthrough is opt-in: supply a product-issued ticket. It does not mint authentication credentials.
				</p>
			</div>
			<form
			...
			>
				...
				<div className="flex flex-wrap gap-3">
					...
					<Button type="button" variant="outline" onClick={() => void triggerExample()}>
						Trigger example job
					</Button>
				</div>
			</form>
			<p role="status" className="text-sm text-muted-foreground">
				{connected ? "Connected; API-backed queries invalidate on event or reconnect." : "Disconnected"}
			</p>
		</main>
	);
}
