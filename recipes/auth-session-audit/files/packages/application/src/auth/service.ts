...
type AuthDatabase = {
	...
	authSession: {
		...
		findUnique(args: { where: { tokenHash: string } }): Promise<SessionRecord | null>;
		update(args: { where: { id: string }; data: Record<string, unknown> }): Promise<SessionRecord>;
	};
};
...
export function createAuthService(input: { db: AuthDatabase; sessionTtlSeconds: number; now?: () => Date; tokenFactory?: () => string }) {
	...
	const safeSession = async (token: string) => {
		...
		await input.db.authSession.update({
			where: { id: session.id },
			data: { lastUsedAt: current, ...(shouldRenew ? { expiresAt: new Date(current.getTime() + input.sessionTtlSeconds * 1000) } : {}) },
		});
