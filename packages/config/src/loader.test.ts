...
function withConfigDirectory(modules: Record<string, string>, run: (configDirectory: string) => void) {
...
test("interpolates present values, defaults, and explicit empty defaults", () => {
	withConfigDirectory({ defaults: "required: ${NAME}\ndefault: ${PORT:-4101}\noptional: ${OPTIONAL:-}\n" }, (configDirectory) => {
		expect(
			loadConfig({
				configDirectory,
				modules: ["defaults"],
...
test("interpolates only once and preserves non-interpolated scalar types", () => {
	withConfigDirectory({ values: "value: ${VALUE}\nport: 4101\nenabled: true\nempty: null\n" }, (configDirectory) => {
		expect(
			loadConfig({
				configDirectory,
				modules: ["values"],
