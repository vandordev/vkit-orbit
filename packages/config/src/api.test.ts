...
const loadApiEnvironment = (environment: Record<string, string | undefined>) =>
	loadConfig({ configDirectory, modules: ["base", "api", "storage"], environment }) as Record<string, string | undefined>;
...
test("maps the public server URL for OpenAPI documentation", () => {
	expect(
		createApiConfig(loadApiEnvironment({ DATABASE_URL: "postgresql://db", OPENAPI_SERVER_URL: "https://api.example.com" }))
			.openapiServerUrl,
...
test("requires both documentation credentials when either is configured", () => {
	expect(() =>
		createApiConfig(loadApiEnvironment({ DATABASE_URL: "postgresql://db", NODE_ENV: "test", OPENAPI_BASIC_AUTH_USERNAME: "docs" })),
	).toThrow();
});
