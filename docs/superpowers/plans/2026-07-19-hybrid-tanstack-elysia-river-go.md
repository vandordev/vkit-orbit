...
Run: `go test ./apps/migrate -run TestRunAppliesPrismaBeforeRiver -count=1`
...
```yaml
install:
  cmds: [bun install --ignore-scripts, go mod download]
migrate:
  cmds: [go run ./apps/migrate]
dev:worker:
  cmds: [go tool air -c dev/air/worker.toml]
test:go:
  cmds: [go test ./... -count=1]
build:go:
  cmds: [go build ./apps/worker ./apps/migrate]
doctor:
  cmds:
...
test:
  cmds: [bun test apps/api apps/web apps/scheduler apps/realtime packages, go test ./... -count=1]
quality:
  cmds: [task test, bun run lint, bun run check-types, go vet ./...]
build:
  cmds: [bun run build, task build:go]
```
...
Run: `go mod tidy && go test ./apps/migrate -count=1 && task --list`
...
Run: `bun test apps/web/vite.config.test.ts apps/web/src/routes/__root.test.tsx`
...
Run: `bun install && bun test apps/web/vite.config.test.ts apps/web/src/routes/__root.test.tsx && bun run --cwd apps/web check-types`
...
Run: `bun test apps/web/src/routes/api.$.test.ts`
...
Run: `bun test apps/api/src/app.test.ts apps/web/src/routes/api.$.test.ts && bun run --cwd apps/web check-types`
...
Run: `bun test packages/queue/src/river.test.ts`
...
Run: `bun test packages/queue apps/scheduler && bun run --cwd packages/queue check-types && bun run --cwd apps/scheduler check-types`
...
Run: `bun test packages/queue/src/example-realtime-notification.test.ts apps/api/src/routes/examples.test.ts apps/scheduler/src/schedules.test.ts`
...
Run: `bun test packages/queue apps/api apps/scheduler`
...
Run: `go test ./internal/notify ./internal/worker ./apps/worker -count=1`
...
Run: `gofmt -w internal apps/worker && go test ./internal/notify ./internal/river ./internal/worker ./apps/worker -count=1`
...
Run: `bun test apps/api/src/routes/internal-notifications.test.ts`
...
Run: `bun test apps/api packages/realtime && bun run --cwd apps/api check-types`
...
Run: `bun test apps/realtime/src/server.test.ts apps/web/src/lib/realtime.test.ts`
...
Run: `bun test apps/realtime packages/realtime apps/web/src/lib/realtime.test.ts apps/web/src/routes/examples/realtime.test.tsx && bun run --cwd apps/realtime check-types`
...
Run: `bun test scripts/check-taskfile.test.ts`
...
Run: `bun test scripts/check-taskfile.test.ts && docker compose config && task doctor`
...
```bash
bun test apps/api packages/queue apps/scheduler packages/realtime apps/realtime apps/web
go test ./... -count=1
```
...
docker compose --profile jobs --profile realtime up --build -d
docker compose down
```
