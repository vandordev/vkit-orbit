...
```ts
expect(Object.keys((route.Route as any).options.server.handlers).sort()).toEqual([
	"DELETE",
	"GET",
	"HEAD",
	"OPTIONS",
...
```ts
expect(createMetadata({ title: "Pricing", description: "Plans", pathname: "/pricing" })).toMatchObject({
	meta: [
...
expect(createMetadata({ title: "Vkit Orbit", description: "Home", image: "/orbit.png" })).toMatchObject({
	meta: [{ title: "Vkit Orbit" }, { property: "og:image", content: "/orbit.png" }],
	links: [],
});
```
