...
- [ ] Run `bun test packages/config/src/api.test.ts apps/web/vite.config.test.ts`; expect failures against the old `4101` and duplicate plugin.
...
- [ ] Run `go test ./apps/migrate ./internal/river -count=1`; expect the exact-command and schema tests to fail.
...
- [ ] Re-run focused Go tests and `task migrate` against Compose PostgreSQL. Commit `fix(river): isolate migration schema`.
...
- [ ] Run `bun test scripts/taskfile.test.ts internal/notify`; expect failures.
- [ ] Update `Taskfile.yml` to launch migrate through `bun --env-file=.env run packages/config/src/run.ts --modules base -- go run ./apps/migrate`. In `NewNotifier`, trim the trailing slash and then trim one `/api/internal/worker-events` suffix before `Notify` appends it.
- [ ] Re-run `bun test scripts/taskfile.test.ts` and `go test ./internal/notify -count=1`; expect PASS. Commit `fix(ops): harden migration and notifier inputs`.
...
- [ ] Run `bun test scripts/dockerfiles.test.ts`; expect failures.
- [ ] Implement the two multi-stage changes while retaining Bun/Go versions, non-root web runner, and existing runtime commands.
- [ ] Run `bun test scripts/dockerfiles.test.ts`, `task quality`, `task build`, then Compose web/migration smoke checks. Commit `build: minimize hybrid runtime images`.
